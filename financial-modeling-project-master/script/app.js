...
const MARKET_FLAGS = {
  'US':'🇺🇸','London':'🇬🇧','Japan':'🇯🇵','Australia':'🇦🇺','Kazakhstan':'🇰🇿'
};
...
function renderTableHeader() {
  ...
  if (asset === 'etf') {
    cells = [
      ...
      th('total_assets','AUM', true), th('nav_price','NAV'), th('expense_ratio_pct','Expense %'),
      th('yield_pct','Yield %'), th('ytd_return_pct','YTD %'), th('three_year_return_pct','3Y Avg %'),
      th('five_year_return_pct','5Y Avg %'), th('beta_3y','Beta'), th('volume','Volume'),
      th('average_volume','Avg Volume'), th(null,'52W Range'), th(null,'Дата источника')
    ];
  } else if (asset === 'bond') {
    cells = [
      th('name','Облигация'), th('market','Рынок'), th('issuer','Эмитент'), th('bond_type','Тип'),
      th('maturity','Срок'), th('yield_pct','Yield %', true), th(null,'Валюта'), th(null,'Дата источника')
    ];
  } else {
    cells = [
...
function stockNameCell(b) {
  ...
  return `<div class="tc">
    <input type="checkbox" class="selck" data-ticker="${b.ticker}"
      onclick="event.stopPropagation();toggleSel('${b.ticker}')"
      ${selectedSet.has(b.ticker) ? 'checked' : ''} title="Выбрать для экспорта в Excel" style="accent-color:#1D6F42">
    <input type="checkbox" class="cmpck" data-ticker="${b.ticker}"
      onclick="event.stopPropagation();toggleCmp('${b.ticker}')"
      ${cmpSet.has(b.ticker) ? 'checked' : ''} title="Сравнить (до ${CMP_MAX})">
...
function renderEtfRow(b) {
  return `<tr>
    <td>${simpleNameCell(b)}</td>
    <td>${marketLabel(b.market)}</td>
    <td><span class="stag" title="${b.fund_family || ''}">${b.fund_category || b.sector || 'ETF'}</span></td>
    <td>${priceCell(b)}</td>
    <td class="bignum">${fBusd(b, 'total_assets')}</td>
    <td class="bignum">${navCell(b)}</td>
    <td>${b.expense_ratio_pct != null ? fP(b.expense_ratio_pct) : '<span class="vn">N/A</span>'}</td>
    ...
    <td class="bignum">${b.average_volume != null ? fBraw(Number(b.average_volume)) : '<span class="vn">N/A</span>'}</td>
    <td>${w52Cell(b)}</td>
    <td class="srcdate">${b.source_date || '—'}</td>
  </tr>`;
}
...
function renderBondRow(b) {
  return `<tr>
    ...
    <td>${b.maturity || '—'}</td>
    <td style="font-family:var(--mono);font-weight:700;color:var(--a)">${b.yield_pct != null ? fP(b.yield_pct) : 'N/A'}</td>
    <td>${b.currency || '—'}</td>
    <td class="srcdate">${b.source_date || '—'}</td>
  </tr>`;
}
...
function updateStats(data) {
  ...
  document.getElementById('st4').textContent = rows.filter(b => b.error).length;
  ...
  if (asset === 'etf') {
    l1.textContent = 'Avg Yield'; l2.textContent = 'Avg Expense'; l3.textContent = 'Avg 3Y Return';
    s2.textContent = '% годовых расходов'; s3.textContent = '% средняя'; v1n.textContent = 'ETF';
    const y = avgField(ok,'yield_pct'), e = avgField(ok,'expense_ratio_pct'), r3 = avgField(ok,'three_year_return_pct');
    v1.textContent = y == null ? '—' : y.toFixed(2)+'%';
    v2.textContent = e == null ? '—' : e.toFixed(2)+'%';
    v3.textContent = r3 == null ? '—' : r3.toFixed(1)+'%';
  } else if (asset === 'bond') {
    l1.textContent = 'Max Yield'; l2.textContent = 'Avg Yield'; l3.textContent = 'Benchmarks';
    s2.textContent = '% U.S. Treasury'; s3.textContent = 'доступно'; v1n.textContent = 'Treasury';
    const ys = ok.map(b => Number(b.yield_pct)).filter(Number.isFinite);
    v1.textContent = ys.length ? Math.max(...ys).toFixed(2)+'%' : '—';
    v2.textContent = ys.length ? (ys.reduce((a,v)=>a+v,0)/ys.length).toFixed(2)+'%' : '—';
    v3.textContent = String(ok.length);
  } else {
    ...
    if (scored.length) {
      const best = [...scored].sort((a,b) => (b.score_pct||0)-(a.score_pct||0))[0];
      v1.textContent = best.score_pct+'%'; v1n.textContent = best.ticker;
  ...
  // Sector controls are only meaningful for operating companies.
  const sel = document.getElementById('ssel');
  if (asset === 'stock') {
    const secs = [...new Set(rows.map(b => b.sector).filter(Boolean))].sort();
    const cur = sel.value;
...
function setSortOptions(asset) {
  ...
  if (asset === 'etf') {
    sel.innerHTML = '<option value="total_assets">AUM</option><option value="price">Цена</option><option value="yield_pct">Yield %</option><option value="expense_ratio_pct">Expense %</option><option value="three_year_return_pct">3Y Return</option>';
    sortState = { key: 'total_assets', dir: -1 };
  } else if (asset === 'bond') {
    sel.innerHTML = '<option value="yield_pct">Yield %</option><option value="name">Название</option><option value="maturity">Срок</option>';
    sortState = { key: 'yield_pct', dir: -1 };
  } else {
    sel.innerHTML = '<option value="score_pct">Score ↓</option><option value="market_cap">Mkt Cap</option><option value="pe_ratio">P/E</option><option value="pb_ratio">P/B</option><option value="roe_pct">ROE %</option><option value="net_income">Net Income</option><option value="price">Цена</option>';
    sortState = { key: 'score_pct', dir: -1 };
  }
}
...
function onAssetMarketChange() {
  ...
  if (asset === 'bond') {
    // Current bond feed is the official U.S. Treasury curve.
    marketEl.value = 'US';
    marketEl.disabled = true;
    tk.placeholder = 'UST-2Y UST-10Y UST-30Y … (пусто = вся Treasury curve)';
  } else {
...
// Initialise new controls without changing the existing loading flow.
