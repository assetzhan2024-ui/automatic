...
def simulate_portfolio_gbm(
    ...
    *,
    ...
    for day in range(1, steps + 1):
        ...
        if day % checkpoint == 0 or day == steps:
    ...
    p5, p50, p95 = np.percentile(terminal_portfolio, [5, 50, 95])
    risk = _loss_tail_metrics(terminal_portfolio, start)
    p05_return_pct = (float(p5) / start - 1.0) * 100.0

    # Asset-level terminal distributions help a beginner understand which
    # selected assets drive the total portfolio outcome.
    asset_scenarios = []
...
def simulate_portfolio_bootstrap(
    ...
    *,
    ...
    chart_values = {day: [] for day in checkpoints}
    ...
    while pos < simulations:
        ...
        # Buy-and-hold policy, identical to GBM: weights are set only at t=0.
        # Assets then evolve independently along each sampled joint-return path,
        # so portfolio weights are allowed to drift naturally over time.
    ...
    for i, ticker in enumerate(names):
        ...
        if initial_i <= 0:
            # Markowitz may assign 0% to an asset. Keep it visible in the
            # per-asset table, but do not run loss percentages with a zero
            # denominator. Its scenario amounts and downside risk are exactly 0.
...
def analyze_portfolio(
    ...
    if objective not in {"max_sharpe", "min_variance"}:
        raise ValueError("Цель оптимизации должна быть max_sharpe или min_variance")
    ...
    if not np.all(np.isfinite(raw_cov)):
        raise ValueError("Матрица covariance содержит невалидные значения")
    ...
    weights = gmv.copy() if objective == "min_variance" else max_sharpe_weights.copy()
    ...
    return {
        ...
        "portfolio": {
            ...
            "method": "Markowitz Efficient Frontier + " + ("Maximum Sharpe" if objective == "max_sharpe" else "Minimum Variance"),
            "objective": objective,
        },
        ...
        **({"_historical_returns": [
        ...
        "methodology": [
            ...
            "Annual covariance: pairwise historical USD-normalized daily-return covariance × 252, then projected to a numerically PSD matrix; displayed correlation is derived from that same PSD covariance.",
            "Markowitz efficient frontier: minimum portfolio variance for a grid of target returns, long-only.",
            "No artificial concentration cap is imposed: each long-only selected asset may receive any weight from 0% to 100%.",
