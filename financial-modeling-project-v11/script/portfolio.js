...
function openPortfolioBuilder() {
  ...
  document.getElementById('portfolio-mi').innerHTML=`
    ...
    <div class="portfolio-form-grid">
      ...
      <label class="portfolio-field"><span>Цель оптимизации</span><select id="portfolio-objective"><option value="max_sharpe">Maximum Sharpe — лучшее соотношение доходности и риска</option><option value="min_variance">Minimum Variance — минимальный исторический риск</option></select></label>
...
async function calculatePortfolio(){
  ...
  const params=new URLSearchParams({assets:[...cmpSet].join(','),amount:String(amount),objective,concentration_mode:concentrationMode});
  try{
  ...
  document.getElementById('portfolio-result').innerHTML=`
    <div class="portfolio-result-card">
      ...
      <div class="portfolio-model-meta">
        <span>Markowitz: <strong>${d.portfolio?.objective==='min_variance'?'Minimum Variance':'Maximum Sharpe'}</strong></span>
      ...
      <div class="portfolio-human-text"><strong>Главная идея:</strong> Markowitz отвечает только на вопрос <em>«как разделить ваш капитал между выбранными активами?»</em>. Он не обещает прибыль. После получения весов этот капитал передаётся в Monte Carlo для оценки возможного будущего диапазона.</div>
      ...
      <div class="event-inline-callout">
        <div><div class="forecast-callout-title">📅 Рядом с портфелем: событийный анализ</div><div class="forecast-callout-text">Для выбранных акций проверим реакцию вокруг даты квартального/годового отчёта: −10…0…+10 торговых дней, abnormal return, CAR и статистическую значимость.</div></div>
        <button class="btn event-run-btn" onclick="openEventStudy()">Событийный анализ</button>
      </div>
