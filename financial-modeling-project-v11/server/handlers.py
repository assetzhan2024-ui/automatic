...
from config.markets import (
    ...
    ETF_MARKET_TICKERS, BOND_MARKET_TICKERS, market_tickers, asset_type_for_ticker,
)
...
def handle_event_study(qs: dict) -> tuple[int, dict]:
    ...
    try:
        ...
        if not event_date:
            ...
            if not dates:
                raise ValueError("Не удалось автоматически найти даты отчётности. Укажите дату вручную.")
