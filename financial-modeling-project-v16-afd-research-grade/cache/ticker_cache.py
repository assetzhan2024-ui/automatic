...
def _do_fetch(tickers: list, asset_type: str = "stock") -> None:
    n = len(tickers)
    with _cache_lock:
        existing = {str(r.get("ticker") or "").upper(): r for r in _cache.get("data", []) if r.get("ticker")}
        _cache.update({"status": "loading", "progress": 0, "total": n, "skipped": 0})

    # Slots preserve requested order for this batch. Existing categories stay in
    # cache, so automatic stocks → ETFs → bonds hydration accumulates one ready
    # dashboard universe instead of replacing the previous batch.
    results = [None] * n
    ...
    def _worker(idx: int, sym: str) -> None:
        ...
        with _cache_lock:
            ...
            merged.update({str(r.get("ticker") or "").upper(): r for r in results if r is not None})
            _cache["data"] = list(merged.values())
