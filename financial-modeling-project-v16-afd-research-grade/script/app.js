...
const PAGE_SIZE  = 25;
const CMP_MAX    = 12;
const CMP_COLORS = ['var(--a)', 'var(--a2)', 'var(--g)', 'var(--y)', 'var(--r)', 'var(--o)', '#22c55e', '#38bdf8', '#e879f9', '#f59e0b', '#a3e635', '#f43f5e'];
...
let curPage      = 1;
let pollTimer    = null;
let cmpSet       = new Set();   // единый выбор для сравнения и Portfolio Builder
...
function assetBadge(b) {
  if (b.asset_type === 'bond') return '<span class="stag" style="color:var(--a)">BOND</span>';
  if (b.asset_type === 'etf') return '<span class="stag" style="color:var(--y)">ETF</span>';
  return scoreBadge(b.score_pct || 0);
}

const MARKET_FLAGS = {
  'US':'🇺🇸','London':'🇬🇧','France':'🇫🇷','Japan':'🇯🇵','Australia':'🇦🇺','Kazakhstan':'🇰🇿'
};
function marketLabel(m) { return `${MARKET_FLAGS[m] || '🌐'} ${m || 'Other'}`; }

// Sanitise ticker string for use in DOM ids (dots → underscores, etc.)
const safeId = t => t.replace(/[^a-zA-Z0-9_]/g, '_');

function currentAsset() {
  const el = document.getElementById('asel');
  return el ? el.value : 'stock';
}

function th(key, label, _defaultActive=false) {
  if (!key) return `<th>${label}</th>`;
  const active = sortState.key === key;
  const arrow = active ? (sortState.dir < 0 ? '↓' : '↑') : '↕';
  return `<th onclick="sb('${key}')"${active ? ' class="act"' : ''}>${label} <span class="arr" id="a-${key}">${arrow}</span></th>`;
}
...
function w52Cell(b) {
  return b.week52_low != null && b.week52_high != null
...
function simpleNameCell(b) {
  return `<div class="tc"><div class="av">${ini(b.name)}</div><div>
    <div class="tn">${b.name || b.ticker}</div>
    <div class="td2">${b.ticker}${b.currency ? ' · ' + b.currency : ''}</div>
  </div></div>`;
}

function renderStockRow(b) {
  const r = b.ratings || {};
  return `<tr onclick="showMod('${b.ticker}')">
    <td>${stockNameCell(b)}</td>
    ...
    <td class="bignum">${b.eps_trailing_usd != null ? '$'+fN(b.eps_trailing_usd) : b.eps_trailing != null ? fN(b.eps_trailing)+' '+fCurSym(b.currency) : '<span class="vn">N/A</span>'}</td>
    <td>${w52Cell(b)}</td>
...
function navCell(b) {
  if (b.nav_price == null) return '<span class="vn">N/A</span>';
  if (b.nav_price_usd != null) return `$${fN(b.nav_price_usd,2)}`;
  return `${fN(b.nav_price,2)} ${fCurSym(b.currency)}`;
}
...
function renderTable(data) {
  ...
  const colCount = renderTableHeader();
  const asset = currentAsset();

  if (!data.length) {
    tb.innerHTML = `<tr><td colspan="${colCount}" style="text-align:center;padding:36px;color:var(--sub)">Нет результатов</td></tr>`;
    renderPager(0);
    document.getElementById('clabel').textContent = `0 / ${allData.length}`;
    return;
  }
  ...
  tb.innerHTML = slice.map(b => asset === 'bond' ? renderBondRow(b) : asset === 'etf' ? renderEtfRow(b) : renderStockRow(b)).join('');
  renderPager(total);
  document.getElementById('clabel').textContent = `${total} / ${allData.filter(b => (b.asset_type || 'stock') === asset).length}`;
}
...
function avgField(rows, key) {
  const vals = rows.map(b => Number(b[key])).filter(v => Number.isFinite(v));
...
async function loadChartInto(ticker, containerId) {
  ...
  const cleanPairs=(cd.dates||[]).map((d,i)=>({date:d,close:Number((cd.closes||[])[i])}))
    .filter(x=>x.date && Number.isFinite(x.close) && x.close>0);
  if(cleanPairs.length<2){
    wrap.innerHTML=`<div class="chart-na">📊 Недостаточно валидных положительных цен для ${ticker}</div>`;
    return;
  }
  const closes=cleanPairs.map(x=>x.close);
  const dates=cleanPairs.map(x=>x.date);
  ...
  const coverageDays=Number(cd.coverage_days ?? ((new Date(dates[n-1])-new Date(dates[0]))/86400000));
  const full52=cd.full_52w===true || (coverageDays>=300 && n>=150);
  const periodTitle=full52?'52 недели · Цена закрытия':`Доступный период · ${Math.max(0,Math.round(coverageDays))} дней`;
  ...
  wrap.innerHTML = `
    <div class="chart-header">
      <div>
        <div class="chart-title">${periodTitle}</div>
      </div>
      <div class="chart-stats">
        ...
        <div class="chart-stat">
          ...
          <div class="chart-stat-v">${full52?fPrice({price:Math.min(...closes), currency}):'N/A'}</div>
        </div>
        <div class="chart-stat">
          <div class="chart-stat-l">52W High</div>
          <div class="chart-stat-v">${full52?fPrice({price:Math.max(...closes), currency}):'N/A'}</div>
        </div>
      </div>
    </div>
    ...
    ${full52?'':`<div class="portfolio-warning">⚠ Источник дал только ${Math.round(coverageDays)} дней истории. 52W Low/High скрыты, чтобы короткий период не выдавался за 52 недели.</div>`}
  `;
...
function showMod(ticker) {
  ...
  document.getElementById('mi').innerHTML = `
    ...
    <div style="display:flex;gap:8px;margin-bottom:14px;flex-wrap:wrap">
      ...
      <a href="https://www.investing.com/search/?q=${encodeURIComponent(b.name||b.ticker)}" target="_blank" rel="noopener"
        ...
        📈 Investing.com ↗</a>
      <button type="button" onclick="openSimilarCompanies('${ticker}')"
        style="font-family:var(--mono);font-size:10px;color:var(--g);padding:4px 10px;
        border:1px solid rgba(0,229,160,.35);border-radius:5px;background:rgba(0,229,160,.06);cursor:pointer">
        🔎 Сравнить похожие компании</button>
    </div>
    ...
    <div class="msec" style="margin-top:18px">Issuer Research</div>
    <!-- Tab switcher -->
    <div style="display:flex;gap:2px;margin:8px 0 0;background:var(--surface);border-radius:8px;padding:3px;border:1px solid var(--border)">
      ...
      <button id="tab-trend-${safeId(ticker)}" onclick="switchTab('${safeId(ticker)}','trend')"
        style="flex:1;padding:6px;background:none;border:none;border-radius:6px;
        font-family:var(--mono);font-size:9px;color:var(--sub);cursor:pointer;text-transform:uppercase;letter-spacing:.06em">
        Fundamental Trend
      </button>
    ...
    <!-- TAB: Fundamental Trend Engine -->
    <div id="tabpanel-trend-${safeId(ticker)}" data-ticker="${ticker}" style="display:none">
      <div id="trend-ctn-${safeId(ticker)}">
        <div class="chart-loading">⏳ Нажмите вкладку для расчёта трендов…</div>
      </div>
    </div>

    <!-- CAPM is issuer-level research, kept separate from the four statement tabs. -->
    <div class="capm-shell" id="capm-shell-${safeId(ticker)}">
      <div class="msec" style="margin-top:18px">CAPM & Required Return</div>
      <div id="capm-ctn-${safeId(ticker)}" data-ticker="${ticker}" data-market="${b.market || ''}">
        <div class="chart-loading">⏳ Загрузка локального CAPM…</div>
      </div>
    </div>
  ...
  setTimeout(() => { if (typeof loadCapmPanel === 'function') loadCapmPanel(ticker, b.market, `capm-ctn-${safeId(ticker)}`); }, 80);
}
...
function switchTab(safeTickerId, tab) {
  const tabs   = ['mult', 'hist', 'fin', 'trend'];
  ...
  if (tab === 'trend') {
    const trendCtn = document.getElementById(`trend-ctn-${safeTickerId}`);
    if (trendCtn && trendCtn.querySelector('.chart-loading')) {
      const panel = document.getElementById(`tabpanel-trend-${safeTickerId}`);
      const ticker = panel?.dataset.ticker;
      if (ticker && typeof loadFundamentalTrends === 'function') loadFundamentalTrends(ticker, `trend-ctn-${safeTickerId}`);
    }
  }
}
...
function openCompare() {
  if (cmpSet.size < 2) { alert('Выберите минимум 2 актива чекбоксами в таблице'); return; }
  ...
  const cards = recs.map((b, i) => `
    <div class="cmp-ticker-card">
      <div class="av" style="width:36px;height:36px;font-size:12px;border-radius:8px;
        margin:0 auto 6px;background:${CMP_COLORS[i % CMP_COLORS.length]};color:var(--bg)">${ini(b.name)}</div>
      <div style="font-family:var(--disp);font-size:14px;letter-spacing:.03em;line-height:1.1">${b.name||b.ticker}</div>
      <div style="font-family:var(--mono);font-size:9px;color:${CMP_COLORS[i % CMP_COLORS.length]};margin:3px 0">${b.ticker}</div>
      ${regBadge(b.region)}
      <div style="margin-top:5px">${scoreBadge(b.score_pct||0)}</div>
    </div>`).join('');

  const thCells = recs.map((b, i) =>
    `<th style="color:${CMP_COLORS[i % CMP_COLORS.length]};font-family:var(--mono);font-size:9px">${b.ticker}</th>`
  ).join('');
  ...
  function makeRows(defs) {
    return defs.map(([label, key, fmt, higherBetter]) => {
      ...
      const cells = vals.map((v, i) => {
        ...
        const col = ratings[i] ? clr(ratings[i]) : (v != null ? CMP_COLORS[i % CMP_COLORS.length] : 'var(--sub)');
...
function onAssetMarketChange() {
  const asset = currentAsset();
  const marketEl = document.getElementById('msel');
  const market = marketEl.value;
  const tk = document.getElementById('tkinput');
  ...
  const xls = document.getElementById('xls-wrap');

  setSortOptions(asset);
  qsel.value = 'all'; rsel.value = 'all'; ssel.value = 'all';
  qsel.disabled = asset !== 'stock';
  rsel.disabled = asset !== 'stock';
  ssel.disabled = asset !== 'stock';
  if (legend) legend.style.display = asset === 'stock' ? 'flex' : 'none';
  if (xls) xls.style.display = asset === 'stock' ? '' : 'none';

  marketEl.disabled = false;
  if (asset === 'bond') {
    tk.placeholder = market === 'All'
      ? 'UST-10Y AAPL-2030-4.200 BX60 AYUHE BIGDb11 … (пусто = все облигации)'
      : `Облигации ${market} (пусто = весь выбранный рынок)`;
  } else {
    tk.placeholder = asset === 'etf'
      ? (market === 'All' ? 'SPY VUSA.L 1306.T VAS.AX … (пусто = все ETF)' : `ETF ${market} (пусто = весь выбранный рынок)`)
      : (market === 'All' ? 'AAPL 7203.T BHP.AX SHEL.L … (пусто = все рынки)' : `Акции ${market} (пусто = весь выбранный рынок)`);
  }
...
async function startFetch() {
  ...
  const asset   = document.getElementById('asel').value;
  const market  = document.getElementById('msel').value;
  ...
  try {
    ...
    await fetch('/api/fetch?' + params.toString(), { method: 'POST' });
  } catch (e) {}
  poll();
}
...
function clearAllData() {
  ...
  ['st0','st1','st1n','st2','st3'].forEach(id => {
...
function universeMarketMap(groups){
  const out={};Object.entries(groups||{}).forEach(([market,tickers])=>(tickers||[]).forEach(t=>out[String(t).toUpperCase()]=market));return out;
}
async function seedDashboardUniverse(){
  try{
    const u=await (await fetch('/api/tickers')).json();
    const sm=universeMarketMap(u.markets),em=universeMarketMap(u.etf_markets),bm=universeMarketMap(u.bond_markets);
    const placeholders=[];
    (u.stocks||[]).forEach(t=>placeholders.push({ticker:t,name:t,asset_type:'stock',market:sm[String(t).toUpperCase()]||'US',loading_placeholder:true,ratings:{},region_medians:{}}));
    (u.etfs||[]).forEach(t=>placeholders.push({ticker:t,name:t,asset_type:'etf',market:em[String(t).toUpperCase()]||'US',loading_placeholder:true}));
    (u.bonds||[]).forEach(t=>placeholders.push({ticker:t,name:t,asset_type:'bond',market:bm[String(t).toUpperCase()]||'US',loading_placeholder:true}));
    if(placeholders.length) applyData(placeholders);
    return u;
  }catch(e){return null;}
}
async function waitServerBatch(){
  for(let i=0;i<900;i++){
    await new Promise(r=>setTimeout(r,500));
    try{const d=await (await fetch('/api/status')).json();syncUI(d);if(d.status!=='loading')return d;}catch(e){}
  }
  return null;
}
async function autoHydrateUniverse(u){
  if(!u)return;
  const batches=[['stock',u.stocks||[]],['etf',u.etfs||[]],['bond',u.bonds||[]]];
  for(const [asset,tickers] of batches){
    if(!tickers.length)continue;
    try{
      const st=await (await fetch('/api/status')).json();if(st.status==='loading')await waitServerBatch();
      await fetch('/api/fetch?'+new URLSearchParams({asset,tickers:tickers.join(',')}),{method:'POST'});
      await waitServerBatch();
    }catch(e){}
  }
}

// Ready-universe UX: tickers are visible immediately; market data hydrate in
// the background. Existing server cache is reused across refreshes.
(async () => {
  try {
    const d = await (await fetch('/api/status')).json();
    if(d.data?.length){syncUI(d);if(d.status==='loading')poll();return;}
    const universe=await seedDashboardUniverse();
    autoHydrateUniverse(universe);
  } catch (e) {}
})();

document.addEventListener('DOMContentLoaded', () => { try { onAssetMarketChange(); } catch (e) {} });
