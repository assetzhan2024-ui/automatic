...
def sec_event_candidates(ticker: str, limit: int = 8) -> list[dict[str, Any]]:
    ...
    try:
        ...
        for i in range(count):
            form, filing_date, accession, primary = forms[i], filing_dates[i], accessions[i], docs[i]
            ...
            period = str(report_date)[:10] if report_date else ""
            period_text = f" · период {period}" if period else ""
            out.append({
                "label": f"{label} · {str(filing_date)[:10]} · {form}{period_text}",
            ...
            if len(out) >= max(limit * 2, 12):
                break
        # Deduplicate identical filing events while preserving distinct reports.
        dedup: dict[tuple[str, str, str], dict[str, Any]] = {}
        for row in out:
            key = (str(row.get("date") or ""), str(row.get("form") or ""), str(row.get("report_date") or ""))
            dedup.setdefault(key, row)
        rows = list(dedup.values())
        rows.sort(key=lambda x: (str(x.get("date") or ""), str(x.get("report_date") or "")), reverse=True)
        return rows[:limit]
    except Exception:
        return []
