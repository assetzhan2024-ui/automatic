...
# Explicit-range USD FX history for historical validation.
def fetch_fx_history_usd_range(currency: str, start: str, end: str):
    """Return daily USD value of one unit of ``currency`` for ``[start, end)``.

    This is deliberately date-bounded so a historical validation window cannot
    accidentally depend on today's rolling five-year history. Missing dates are
    left missing here; callers may forward-fill only from already-known past FX
    observations when aligning exchange calendars.
    """
    import pandas as pd
    ...
    def _series(symbol: str):
        h = fetch_history_range(symbol, start, end, timeout=7)
    ...
    if base == "KZT":
        ...
        if direct is not None and not direct.empty:
            ...
            inverse = _series("USDKZT=X")
            if inverse is None or inverse.empty:
                return None
            s = (1.0 / inverse).replace([float("inf"), float("-inf")], pd.NA).dropna()
    else:
        s = _series(pair)
        if s is None or s.empty:
            # Some Yahoo feeds expose only the reciprocal pair. Try it before
            # failing, and invert explicitly to preserve USD per local unit.
            reciprocal = f"USD{base}=X"
            inv = _series(reciprocal)
