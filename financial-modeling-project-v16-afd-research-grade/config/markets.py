...
US, London, France, Japan, Australia and Kazakhstan.
...
plus curated corporate-bond issues for the supported markets.
"""
...
_ALLOWED_MARKETS = {"US", "London", "France", "Japan", "Australia", "Kazakhstan"}
_DISALLOWED_SUFFIXES = (
    ".DE", ".AS", ".MI", ".MC", ".SW", ".ST", ".HE", ".OL", ".CO", ".BR", ".VI", ".LS", ".IR", ".AT",
...
# Remove ETFs and all exchange listings outside the six supported markets.
...
def _groups(items: list[str]) -> dict[str, list[str]]:
    groups = {m: [] for m in ["US", "London", "France", "Japan", "Australia", "Kazakhstan"]}
    for t in items:
...
def market_tickers(market: str, asset_type: str = "stock") -> list[str]:
    ...
    mk = market if market in {"All", "US", "London", "France", "Japan", "Australia", "Kazakhstan"} else "All"
